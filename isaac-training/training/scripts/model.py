...
class ConvNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv2 = nn.Conv2d(4, 10, 2, 1)
        self.avgpool = nn.AvgPool2d(kernel_size=(2,1), stride=1)
        self.maxpool = nn.MaxPool2d(2, 1)
        # self.bn1 = nn.BatchNorm2d(4)
        self.bn1 = nn.GroupNorm(2, 4)
    ...
    def forward(self, X):
        ...
        return x #None is passed to be compatible with hidden dimensions
...
class LSTMNetVIT(nn.Module):
    ...
    def forward(self, X):
        ...
        out = embeds[1:]
        x2 = self.pxShuffle(out[1])
        x1 = F.interpolate(out[0], size=x2.shape[2:], mode='bilinear', align_corners=True)
...
class ViT(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.encoder_blocks = nn.ModuleList([
            MixTransformerEncoderLayer(1, 32, patch_size=2, stride=1, padding=3, n_layers=2, reduction_ratio=2, num_heads=1, expansion_factor=8),
            MixTransformerEncoderLayer(32, 64, patch_size=2, stride=1, padding=1, n_layers=2, reduction_ratio=2, num_heads=2, expansion_factor=8)
        ])        
        self.decoder = nn.LazyLinear(128)
        # self.nn_fc1 = spectral_norm(nn.LazyLinear(256))
        # self.nn_fc2 = spectral_norm(nn.LazyLinear(3))
        self.up_sample = nn.Upsample(size=(16,24), mode='bilinear', align_corners=True)
        self.pxShuffle = nn.PixelShuffle(upscale_factor=2)
        self.down_sample = nn.Conv2d(48,12,3, padding = 1)
