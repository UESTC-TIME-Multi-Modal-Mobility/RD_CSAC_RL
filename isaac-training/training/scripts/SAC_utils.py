...
Date: 2025-07-29 22:55:26
LastEditors: zdytim zdytim@foxmail.com
LastEditTime: 2025-08-05 11:32:53
FilePath: /u20/NavRL/isaac-training/training/scripts/utils copy.py
...
class CriticInput(nn.Module):
    def forward(self, _feature, action):
        return torch.cat([_feature, action], dim=-1)
...
class ActionPostProcess(nn.Module):
    ...
    def forward(self, action, direction):
        # [-1, 1] -> [-action_limit, action_limit]
        action_scaled = action * self.action_limit
        # 坐标变换
        actions_world = vec_to_world(action_scaled, direction)
...
def make_sac_agent(cfg, train_env, eval_env, device):
    """Make SAC agent."""
    in_keys = ["agents", "observation", "lidar","dynamic_obstacle"]
    action_spec = train_env.action_spec.to(device)
    ...
    # actor_net = nn.Sequential(feature_extractor, actor_extractor)
    ...
    dist_kwargs = {
        # "low": torch.full(action_spec.shape, -1.0, device=action_spec.device),
        # "high": torch.full(action_spec.shape, 1.0, device=action_spec.device),
        "tanh_loc": False,
    }
    actor_module = TensorDictSequential(
        ...
        # TensorDictModule(
        # ActionPostProcess(action_limit),
        # in_keys=["action", ("agents", "observation", "direction")],
    ...
    actor = ProbabilisticActor(
        ...
        # 🎯 如果需要后处理，单独定义
    action_postprocess = TensorDictModule(
        ActionPostProcess(action_limit),
        in_keys=["action", ("agents", "observation", "direction")],
        out_keys=[("agents", "action")]
    )
    # 🎯 完整的 exploration_policy
    ...
    critic_input = TensorDictModule(
        CriticInput(),
        in_keys=["_feature", "action"],
        out_keys=["critic_input"]
    )
    # Q值网络
    qvalue_net = TensorDictModule(
        make_mlp([256, 256, 1]),
        in_keys=["critic_input"],
    ...
    with torch.no_grad(), set_exploration_type(ExplorationType.RANDOM):
        ...
        td["action"] = torch.zeros(action_spec.shape, device=device)
        # dummy_input = observation_spec.zero()
        td = td.to(device)
        exploration_policy(td)
        for net in model:
            net(td)
    return model, model[0]  # model[0] is the actor (exploration policy)
